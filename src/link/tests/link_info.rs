...
#[rustfmt::skip]
#[test]
fn test_link_info_non_null_terminated_string() {
    let data = vec![
    ...
    let expected = RouteNetlinkMessage::NewLink(LinkMessage {
        ...
        nlas: vec![
            LinkAttribute::IfName(String::from("qemu-br1")),
            LinkAttribute::Info(vec![Info::Kind(InfoKind::Bridge)]),
        ],
    });
    let nl_buffer = NetlinkBuffer::new(&data).payload();
    let rtnl_buffer = RouteNetlinkMessageBuffer::new(&nl_buffer);
    let actual = RouteNetlinkMessage::parse_with_param(
        &rtnl_buffer, RTM_NEWLINK).unwrap();
