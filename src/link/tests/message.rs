...
fn link_message_packet_header_read() {
    let packet = LinkMessageBuffer::new(&LINK_MSG_HDR[0..16]);
...
fn link_message_packet_header_build() {
    ...
    assert_eq!(&buf[..], &LINK_MSG_HDR[0..16]);
}

#[test]
fn link_mssage_packet_nlas_read() {
    let packet = LinkMessageBuffer::new(&LINK_MSG_HDR[..]);
    ...
    let parsed = LinkAttribute::parse_with_param(&nla, AF_INET).unwrap();
